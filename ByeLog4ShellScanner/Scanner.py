...
cprint('[•] ByeLog4Shell Scanner for Log4Shell Vulnerability', "green")
...
class Interactsh:
    ...
    def register(self):
        ...
        if 'success' not in res.text:
            raise Exception("Couldnt initiate DNS callback client")
...
def scan_url(url, callback_host):
    ...
    for payload in payloads:
        cprint(f"[•] ByeLog4Shell - Using Payload: {payload}", "cyan")
...
def main():
    ...
    if args.custom_dns_callback_host:
        ...
        cprint(f"[•] ByeLog4Shell - Initiating DNS callback server.")
    ...
    cprint("[%] ByeLog4Shell - Checking for Log4j RCE CVE-2021-44228.", "magenta")
    for url in urls:
        cprint(f"[•] URL: {url}", "magenta")
        scan_url(url, dns_callback_host)

    if args.custom_dns_callback_host:
        cprint("[•] ByeLog4Shell - Payloads sent to all URLs. Custom DNS Callback host is provided, please check your logs to verify the existence of the vulnerability. Exiting.", "cyan")
        return

  
    cprint("[•] ByeLog4Shell - Payloads has been sent to all URLs. Waiting for DNS OOB callbacks.", "cyan")
    cprint("[•] ByeLog4Shell - Waiting...", "cyan")
    time.sleep(int(args.wait_time))
    records = dns_callback.pull_logs()
    if len(records) == 0:
        cprint("[•] ByeLog4Shell - Good Job for the Devs cause the Url is not vulnerable by Log4Shell!", "green")
    else:
        cprint("[!!!] ByeLog4Shell - Uh Oh! The Url is Affected By Log4Shell.", "red")
        for i in records:
...
if __name__ == "__main__":
    try:
        ...
        print("ByeLog4Shell - Canceling Scan...")
        exit(0)